...
| 来路 | 含义 | 现有 |
| --- | --- | --- |
| `extracted` | 自原书文本逐条提取、清洗、标注条目位置 | 2（《穷通宝鉴》120 格、《滴天髓》4 篇） |
...
《子平真詮》未能回原书核对——维基文库无此书（多个词条名皆 404）。其八格的原文
引自 `tianzhi_core/bazi/geju.py` 的注释，原录入者不详，故记为 `packaged`：拆句
成条是本次所做，原文录入不是。待寻得可靠的公版电子本后再逐句核对。

余下典籍尚未逐本精读提取，仍为 `derived` 与 `modeled`。

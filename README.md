...
术数不止八字。`modules` 列出门类，每部书用 `module` 归到其中一门，书架按门类分组。
现在只有八字一门收齐了，六壬、奇门、紫微、六爻、梅花的位置先留着，`ready` 为假。
...
两类都不需要人工建映射表，因为算法里本来就带着出处。

`chapter` 这一类，出处已经写在 `tianzhi_core` 的 evidence 里，篇名就是锚点：

```
geju.month_pattern() → evidence = ('月令午本气丁·七杀', '未见透出', '子平真诠·论用神')
                                                          ^^^^^^^^^^^^^^^^^^
```

`gan-month` 这一类更直接。《穷通宝鉴》本身就是十天干乘十二月令编成的 120 格，
`tianzhi_core/data/tiaohou.json` 跟它逐格对应，所以「日干 + 月支」直接就是条目
编号——辛日生午月，锚点就是 `辛-午`。

需要注意算法侧还有个缺口：调候那一层现在拼的是 `穷通宝鉴·{用神天干}`，拼出来
形如「穷通宝鉴·壬癸」，那是**算出来的结果**，不是出处的位置，定位不到格。要改成
`穷通宝鉴·辛日午月` 才对得上。扶抑、病药、通关几层则还没带书名，`神峰通考·病药说`
只写在注释里没进 evidence。这些补在 tianzhi-core 那边。
